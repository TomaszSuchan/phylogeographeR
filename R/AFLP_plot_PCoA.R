...
AFLP_plot_PCoA <- function(aflp_matrix_path, population_data_path=NULL, choices=c(1,2), remove=2) {
  ...
  populations <- merge(as.data.frame(aflp_matrix$Population),
                       ...
                       by.y="V1")
  scores <- cbind(populations$V2, as.data.frame(scores(PCoA, display = "sites", choices=choices)))
